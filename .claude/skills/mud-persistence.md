...
6. Schema is managed by numbered migration files in `migrations/` — currently 58 files (029 numbered migrations × 2 files each) — and applied via `cmd/migrate/main.go` (using `golang-migrate/migrate`) invoked as `make migrate`.
