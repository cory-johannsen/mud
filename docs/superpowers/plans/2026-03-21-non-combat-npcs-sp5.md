...
**Note:** There is no global "personality defaults table" in `template.go` — personality is a plain YAML string field. The personality default for fixer (REQ-WC-3) is enforced directly in the fixer switch case below by normalising empty to `"cowardly"` and rejecting any other value.
...
**Package**: `package gameserver` (NOT `package gameserver_test` — this package uses white-box testing for access to unexported helpers like `testWorldAndSession` and `testServiceWithNPCMgr`).

Pattern reference: `internal/gameserver/grpc_service_healer_test.go` — copy the `newHealerTestServer` structure exactly. The helpers `testWorldAndSession(t)` and `testServiceWithNPCMgr(t, worldMgr, sessMgr, npcManager)` are defined elsewhere in the `gameserver` package test files; do not redefine them.
...
fixer:
  npc_variance: 1.15
  max_wanted_level: 4  # all 4 levels handled
  base_costs:
    1: 150
    2: 350
    3: 700
