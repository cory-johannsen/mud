...
func TestStartZoneTicks_RespawnIntegration(t *testing.T) {
	...
	svc := newTestGameServiceServer(
		...
		respawnMgr, nil, nil, nil, nil, nil,
...
func joinWorld(t *testing.T, stream gamev1.GameService_SessionClient, uid, username string) *gamev1.RoomView {
	...
	// The server sends exactly three events on join:
	//   1. RoomView (initial room state)
	//   2. HotbarUpdateEvent (initial hotbar state for REQ-HB-12)
	//   3. GameConfig (server-side client configuration, REQ-CNT-2)
	// Consume all three so callers start with a clean stream.
...
func TestHandleUse_AppliesConditionWhenConditionIDSet(t *testing.T) {
	...
	event, err := svc.handleUse("u_use_cond", "power-surge", "", 0, 0)
	require.NoError(t, err)
	require.NotNil(t, event)

	useResp := event.GetUseResponse()
	require.NotNil(t, useResp, "expected UseResponse")
	assert.Equal(t, "You surge with power!", useResp.Message, "message must be ActivateText only; no condition name parenthetical (REQ-BUG29)")
...
func TestHandleUse_NoConditionAppliedWhenConditionIDEmpty(t *testing.T) {
	...
	event, err := svc.handleUse("u_use_no_cond", "quick-strike", "", 0, 0)
	require.NoError(t, err)
	require.NotNil(t, event)
...
func TestProperty_HandleUse_ConditionIDEmpty_NoConditionApplied(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		...
		_, err = svc.handleUse("prop_uid_use", "prop-feat", "", 0, 0)
		if err != nil {
...
func TestHandleRaiseShield_InCombat_InsufficientAP(t *testing.T) {
	...
	worldMgr, sessMgr := testWorldAndSession(t)
	logger := zaptest.NewLogger(t)
	combatHandler := NewCombatHandler(combat.NewEngine(), npc.NewManager(), sessMgr, nil, nil, 0, condReg, worldMgr, nil, nil, nil, nil, nil, nil, nil)
	svc := newTestGameServiceServer(
		worldMgr, sessMgr,
		command.DefaultRegistry(),
		NewWorldHandler(worldMgr, sessMgr, npc.NewManager(), nil, nil, nil),
...
func TestHandleFirstAid_InCombat_InsufficientAP(t *testing.T) {
	...
	worldMgr, sessMgr := testWorldAndSession(t)
	combatHandler := NewCombatHandler(combat.NewEngine(), npc.NewManager(), sessMgr, nil, nil, 0, nil, worldMgr, nil, nil, nil, nil, nil, nil, nil)
	svc := newTestGameServiceServer(
		worldMgr, sessMgr,
		command.DefaultRegistry(),
		NewWorldHandler(worldMgr, sessMgr, npc.NewManager(), nil, nil, nil),
...
func TestHandleFeint_TargetNotFound(t *testing.T) {
	...
	_, err := npcMgr.Spawn(&npc.Template{
		ID: "goblin-tnf", Name: "Goblin", Level: 1, MaxHP: 20, AC: 13, Awareness: 2,
	}, roomID)
	require.NoError(t, err)
...
func TestHandleFeint_Success_RollBelow(t *testing.T) {
	...
	_, err := npcMgr.Spawn(&npc.Template{
		ID: "bandit-rb", Name: "Bandit", Level: 1, MaxHP: 20, AC: 13, Awareness: 15,
	}, roomID)
	require.NoError(t, err)
...
func TestHandleFeint_Success_RollAbove(t *testing.T) {
	...
	inst, err := npcMgr.Spawn(&npc.Template{
		ID: "ganger-ra", Name: "Ganger", Level: 1, MaxHP: 20, AC: 13, Awareness: 5,
	}, roomID)
	require.NoError(t, err)
...
func TestHandleDemoralize_TargetNotFound(t *testing.T) {
	...
	_, err := npcMgr.Spawn(&npc.Template{
		ID: "goblin-dem-tnf", Name: "Goblin", Level: 1, MaxHP: 20, AC: 13, Awareness: 2,
...
func TestHandleDemoralize_RollBelow(t *testing.T) {
	...
	_, err := npcMgr.Spawn(&npc.Template{
		ID: "bandit-dem-rb", Name: "Bandit", Level: 5, MaxHP: 20, AC: 13, Awareness: 5,
		Abilities: npc.Abilities{Brutality: 10, Quickness: 10, Savvy: 10},
	}, roomID)
	require.NoError(t, err)
...
func TestHandleDemoralize_RollAbove(t *testing.T) {
	...
	inst, err := npcMgr.Spawn(&npc.Template{
		ID: "ganger-dem-ra", Name: "Ganger", Level: 1, MaxHP: 20, AC: 13, Awareness: 5,
		Abilities: npc.Abilities{Brutality: 10, Quickness: 10, Savvy: 10},
	}, roomID)
	require.NoError(t, err)
