...
func newAuthHandler(t *testing.T, store AccountStore, gsAddr string) *AuthHandler {
	...
	return NewAuthHandler(store, chars, []*ruleset.Region{}, []*ruleset.Team{}, []*ruleset.Job{}, []*ruleset.Archetype{}, logger, gsAddr, telnetCfg, nil, nil, nil, nil)
}
...
func testGameServer(t *testing.T) string {
	...
	svc := gameserver.NewGameServiceServer(worldMgr, sessMgr, cmdRegistry, worldHandler, chatHandler, logger, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "", nil, nil, nil, nil, nil)
