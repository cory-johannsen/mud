...
func TestHandleFeint_WithTarget(t *testing.T) {
	req, err := HandleFeint([]string{"bandit"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if req == nil {
		t.Fatal("expected non-nil FeintRequest")
	}
...
func TestHandleFeint_NoArgs(t *testing.T) {
	req, err := HandleFeint(nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if req == nil {
...
func TestHandleFeint_Property(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		args := rapid.SliceOf(rapid.String()).Draw(rt, "args")
		req, err := HandleFeint(args)
		if err != nil {
