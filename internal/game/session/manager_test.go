...
func TestManager_AddPlayer(t *testing.T) {
	m := NewManager()
	sess, err := m.AddPlayer("u1", "Alice", "room_a")
...
func TestManager_AddPlayerDuplicate(t *testing.T) {
	m := NewManager()
	_, err := m.AddPlayer("u1", "Alice", "room_a")
	require.NoError(t, err)
	_, err = m.AddPlayer("u1", "Alice2", "room_b")
	assert.Error(t, err)
...
func TestManager_PlayersInRoom(t *testing.T) {
	m := NewManager()
	_, _ = m.AddPlayer("u1", "Alice", "room_a")
	_, _ = m.AddPlayer("u2", "Bob", "room_a")
	_, _ = m.AddPlayer("u3", "Charlie", "room_b")
...
func TestManager_ConcurrentAddRemove(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		go func(i int) {
			...
			_, _ = m.AddPlayer(uid, fmt.Sprintf("Player%d", i), "room_a")
		}(i)
	}
	wg.Wait()
...
func TestManager_ConcurrentMove(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		_, err := m.AddPlayer(fmt.Sprintf("u%d", i), fmt.Sprintf("P%d", i), rooms[0])
		require.NoError(t, err)
	}
...
func TestPropertyRoomOccupancyConsistent(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		for i := 0; i < numPlayers; i++ {
			...
			_, _ = m.AddPlayer(uid, fmt.Sprintf("Player%d", i), rooms[roomIdx])
		}
